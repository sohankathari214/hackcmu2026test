...
def forecast_scenario(profile:Any,state:Any,population_models=None,patient_models=None,episodes=None):
    models=population_models or _load_models(); f=build_feature_vector(profile,_events(profile,state),state.timestamp,state.proposed_action).reindex(FEATURE_NAMES).fillna(0); fc={}
    ...
    return {"status":"ok","scenario":{"action_type":typ,"baseline":typ=="none"},"population_forecast":fc,"personalized_forecast":{},"quantiles":quant,"hypoglycemia_probability":float(models["hypo_120"].predict_proba(f.to_frame().T)[0,1]) if "hypo_120" in models else None,"hyperglycemia_probability":float(models["hyper_120"].predict_proba(f.to_frame().T)[0,1]) if "hyper_120" in models else None,"personalization":{"active":False,"episode_count":0,"alpha":0.,"personal_model_version":None},"support":{"supported":True,"support_type":"real_hupa" if typ=="none" else "synthetic_poc"},"data_quality":data_quality(f,state),"warnings":[warning] if warning else [],"provenance":{"real_physiology":"HUPA-UCM","action_augmentation":"synthetic_poc" if typ!="none" else None},"similar_events":retrieve_similar_events(episodes or [],{},k=10)}
